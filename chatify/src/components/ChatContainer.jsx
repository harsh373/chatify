  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg' >
      ...
      <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-900'>
        ...
        <img src={assets.help_icon} alt="" className='bg-black rounded-2xl max-md:hidden max-w-5' />
      </div>
          ...
          msg ? (
            <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id ? 'flex-row-reverse' : ''}`}>
              {msg.image ? (
                ...
                <p className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg mb-8 break-all bg-black text-white ${msg.senderId === authUser._id ? 'rounded-br-none' : 'rounded-bl-none'}`}>
                  {msg.text}
                </p>
              )}
      ...
      <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
        ...
        <img onClick={handleSendMessage} src={assets.send_button} alt="" className='filter grayscale w-7 cursor-pointer' />
      </div>
