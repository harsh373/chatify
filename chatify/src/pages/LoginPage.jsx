...
const LoginPage = () => {
  ...
  return (
    <div className='min-h-screen bg-cover bg-center flex items-center justify-center gap-10 sm:justify-evenly max-sm:flex-col '>

      <img src={assets.logomain} alt="" className='w-100 rounded-4xl bg-black ' />

      <form onSubmit={onSubmitHandler} className='border-2 bg-white text-black border-gray-900 p-6 flex flex-col gap-8 rounded-lg shadow-lg'>
...

        {
          currState === 'Sign Up' && isDataSubmitted && (
            <textarea onChange={(e) => setBio(e.target.value)} value={bio}
              rows={4} className='p-2 border border-gray-500 rounded-md focus:outline-none' placeholder='Bio is required ' required />
          )
        }

        <button type='submit' className='py-3 bg-gradient-to-r from-black to grey-400 text-white rounded-md cursor-pointer'>
          {currState === 'Sign Up' ? 'Create Account' : 'Login Now'}
        </button>

        <div className='flex items-center gap-2 text-sm text-gray-500'>
          <input type="checkbox" />
          <p>Agree to the terms of use and privacy polichy</p>
        </div>
